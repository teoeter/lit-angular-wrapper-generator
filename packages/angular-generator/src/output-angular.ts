...
export function generateProxies(
   ...
   */
  const angularCoreImports = ['ChangeDetectionStrategy', 'ChangeDetectorRef', 'Component', 'ElementRef', 'Inject'];

  if (includeOutputImports) {
    // Only import EventEmitter, not Output decorator (for JIT mode compatibility)
...
${createImportStatement(componentLibImports, './angular-component-lib/utils.js')}\n`;
  /**
  ...
  const generateTypeImports = () => {
    ...
    // Only append customElementsDir if it's not empty
    importLocation += (isCustomElementsBuild && outputTarget.customElementsDir) ? `/${outputTarget.customElementsDir}` : '';
   ...
   *
   * Skip if customElementsDir is empty (e.g., for Lit components that don't use this pattern)
   */
  if (isCustomElementsBuild && outputTarget.componentCorePackage !== undefined && outputTarget.customElementsDir) {
  ...
  for (let cmpMeta of components) {
    ...
    const componentDefinition = createAngularComponentDefinition(
      ...
      orderedInputs,
      methods,
      isCustomElementsBuild && !!outputTarget.customElementsDir, // Only include custom element imports if customElementsDir is specified
      isStandaloneBuild,
